...
<form name="formularioPago" method="POST" action="{$link->getModuleLink('pagofacil', 'validation', [], true)|escape:'html'}">
    ...
    {/if}
        ...
        <input id="installments" name="installments" type="hidden" value="{Configuration::get('PF_INSTALLMENTS')}">
        <input id="idSucursal" name="idSucursal" type="hidden" value="{Configuration::get('PF_API_BRANCH')}">
    ...
    <p class="cart_navigation" id="cart_navigation">
        ...
        jQuery(document).ready(function($) {
            let installments = jQuery('#installments').val();
            if(installments == 1){
                startMsi();
            }
        });
